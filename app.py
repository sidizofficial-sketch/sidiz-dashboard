...
if 'curr_date' not in st.session_state:
    st.session_state.curr_date = [today - timedelta(days=7), today - timedelta(days=1)]
if 'comp_date' not in st.session_state:
    st.session_state.comp_date = [today - timedelta(days=14), today - timedelta(days=8)]
...
with st.sidebar:
    ...
    # 현재 선택된 날짜 표시
    st.info(f"📊 분석: {st.session_state.curr_date[0]} ~ {st.session_state.curr_date[1]}")
    st.info(f"📊 비교: {st.session_state.comp_date[0]} ~ {st.session_state.comp_date[1]}")
    
    # 캘린더 표시 (버튼 누를 때만)
    if st.session_state.show_calendar:
        new_curr = st.date_input("분석 기간", st.session_state.curr_date, key="curr_input")
        new_comp = st.date_input("비교 기간", st.session_state.comp_date, key="comp_input")
        
        if st.button("✅ 적용"):
            st.session_state.curr_date = new_curr
            st.session_state.comp_date = new_comp
            st.session_state.show_calendar = False
            st.rerun()
...
# 변수 할당
curr_date = st.session_state.curr_date
comp_date = st.session_state.comp_date
...
if len(curr_date) == 2 and len(comp_date) == 2:
    ...
    st.info("💡 사이드바에서 기간을 선택해주세요.")# SIDIZ Dashboard v2.2 - NameError 해결 완료 버전
import streamlit as st
from google.cloud import bigquery
import pandas as pd
import json
